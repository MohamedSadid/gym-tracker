...
8. **Exercise demo** — full-width two-frame loop (when shipped) on top. Under it: **Main** and **Auxiliary** targeted muscles, then how-to tips. First pass: **Barbell bench press**, **Dumbbell bench press**, **Incline barbell press**, and **Incline dumbbell press**. Other built-ins: empty demo until stills exist; muscles still show. Custom exercises: no demo; **Main** is the primary muscle they chose, no auxiliary. Back returns to the list. **Add** only when opened from the picker.
